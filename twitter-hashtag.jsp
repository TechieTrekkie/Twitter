...
<%
	...
	String display_hashtag_ID = request.getParameter("hashID");
	...
	//Get hashtag Text to Display
    String HashtagContentQuery = "SELECT hashtag_content FROM hashtags_list_t WHERE hashtag_ID=?;";

    java.sql.PreparedStatement HashtagContentPS = conn.prepareStatement(HashtagContentQuery); // create a statement
    HashtagContentPS.setString(1, display_hashtag_ID); // set input parameter
    java.sql.ResultSet HashtagContentRS = HashtagContentPS.executeQuery();
    
    while (HashtagContentRS.next())    // Go through all tweets
    {
    	hashtagText=HashtagContentRS.getString("hashtag_content");
    }
    /**********************************************************************/
            ...
            <div class="span4" id="secondary">
                <div class="module mini-profile">
                    ...
                    <form action="twitter-posttweet.jsp" class="posttweet" method="post">
                        ...
                        <input type="hidden" name="referer" value="twitter-hashtag.jsp?hashID=<%=display_hashtag_ID%>"></input>
                ...
                <div class="module">
                    <div class="content-header">
                        <div class="header-inner">
                            <h2 class="js-timeline-title">Tweets Using <%=hashtagText%></h2>
                        </div>
                    </div>
                    ...
                    <%
                        ...
                        String TweetsToShowQuery = "SELECT * FROM tweets_t WHERE (tweet_ID IN (SELECT tweet_ID FROM tweet_hashtags_t WHERE hashtag_ID=?)) AND tweet_ID NOT IN (SELECT reply_ID AS tweet_ID FROM replies_t) ORDER BY publish_date DESC;";

                        java.sql.PreparedStatement TweetsToShowPS = conn.prepareStatement(TweetsToShowQuery); // create a statement
                        TweetsToShowPS.setString(1, display_hashtag_ID); // set input parameter
                        ...
                        while (TweetsToShowRS.next())    // Go through all tweets
                        {
                            ...
                            for(String word : tweetContentArray)//Check for hashtags
                            {
                                ...
                                if(word.length()!=0 && word.charAt(0)=='#')
                                {
                                    ...
                                    if(hashtag_ID.equals(display_hashtag_ID))//Bold hashtag if it is the one being viewed
                                    {
                                        word="<b><a href=\"twitter-hashtag.jsp?hashID="+hashtag_ID+"\">"+word+"</a></b>";
                                    }
                                    else
                                    {
                                ...
                                <div class="content">
                                    ...
                                    <span>
                                        <div class="btn-group" role="group">
                                            ...
                                            <a href="<%if(already_retweeted!=0||tweeter_ID.equals(login_ID)){%>#<%}else{%>twitter-posttweet.jsp?tweet_contents=<%=tweet_content_raw.replaceAll("#", "!!!hash!!!")%>&retweet_id=<%=tweet_ID%>&referer=twitter-hashtag.jsp?hashID=<%=display_hashtag_ID%><%}%>">
                                            ...
                                            <a href="twitter-favoritetweet.jsp?tweet_id=<%=tweet_ID%>&referer=twitter-hashtag.jsp?hashID=<%=display_hashtag_ID%>">
                                            ...
                                            <a href="<%if(retweeter_ID.equals(login_ID)){%>twitter-deletetweet.jsp?tweet_id=<%=retweeter_tweet_ID%>&referer=twitter-hashtag.jsp?hashID=<%=display_hashtag_ID%><%}else{%>twitter-togglefollower.jsp?followed_id=<%=retweeter_ID%>&referer=twitter-hashtag.jsp?hashID=<%=display_hashtag_ID%><%}%>">
                                                <button type="button" class="btn btn-xs btn-default" style="padding-left: 12px; padding-right: 12px;padding-bottom: 2px;padding-top: 2px;width: 55.81818175315857px;">
                            ...
                            <div class="twttr-dialog-footer">
                                <span>
                                    <form action="twitter-posttweet.jsp" class="posttweet" method="post" style="margin-bottom: 5px;">
                                        ...
                                        <input type="hidden" name="referer" value="twitter-hashtag.jsp?hashID=<%=display_hashtag_ID%>#<%=tweet_ID%>replies"></input>
